import { useState, type CSSProperties, type FormEvent } from 'react'
import { useReveal } from '../../hooks/useReveal'
import { KAKAO_URL } from './data'

const fieldWrapStyle: CSSProperties = { display: 'flex', flexDirection: 'column', gap: 8 }
const labelStyle: CSSProperties = { fontSize: 14, fontWeight: 600, color: '#14263F' }
const inputStyle: CSSProperties = {
  border: '1px solid rgba(112,115,124,.28)',
  borderRadius: 6,
  padding: '13px 14px',
  fontSize: 15,
  fontFamily: 'inherit',
  color: '#14263F',
}

export default function Contact() {
  const infoRef = useReveal()
  const formRef = useReveal<HTMLFormElement>()
  const [sent, setSent] = useState(false)

  const submitForm = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    setSent(true)
    e.currentTarget.reset()
  }

  return (
    <div id="contact" style={{ padding: '104px 24px', background: '#F7F5F0' }}>
      <div
        style={{
          maxWidth: 1000,
          margin: '0 auto',
          display: 'grid',
          gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))',
          gap: 56,
        }}
      >
        <div ref={infoRef}>
          <div style={{ fontSize: 13, letterSpacing: '.28em', color: '#C3A15F', fontWeight: 600 }}>
            CONTACT
          </div>
          <h2
            style={{
              fontFamily: "'Noto Serif KR',serif",
              fontSize: 'clamp(26px,4vw,34px)',
              fontWeight: 700,
              color: '#14263F',
              margin: '14px 0 0',
            }}
          >
            상담 문의
          </h2>
          <p style={{ fontSize: 16, color: '#5B6472', lineHeight: 1.8, margin: '18px 0 0' }}>
            간단한 정보를 남겨주시면
            <br />
            빠르게 연락드리겠습니다.
          </p>
          <div
            style={{
              display: 'flex',
              flexDirection: 'column',
              gap: 14,
              marginTop: 32,
              fontSize: 15,
              color: '#14263F',
            }}
          >
            <div>
              <strong style={{ color: '#C3A15F', fontWeight: 700, marginRight: 10 }}>전화</strong>
              010-2763-8554 · 02-6052-8554
            </div>
            <div>
              <strong style={{ color: '#C3A15F', fontWeight: 700, marginRight: 10 }}>팩스</strong>
              02-6052-8555
            </div>
            <div>
              <strong style={{ color: '#C3A15F', fontWeight: 700, marginRight: 10 }}>메일</strong>
              hoon_tax@naver.com
            </div>
            <div>
              <strong style={{ color: '#C3A15F', fontWeight: 700, marginRight: 10 }}>주소</strong>
              서울특별시 송파구 송파동 1905, 201호
            </div>
          </div>
          <div style={{ display: 'flex', gap: 12, marginTop: 32, flexWrap: 'wrap' }}>
            <a
              href="tel:010-2763-8554"
              style={{
                background: '#14263F',
                color: '#F5EFE3',
                fontWeight: 600,
                fontSize: 15,
                padding: '13px 26px',
                borderRadius: 4,
                display: 'inline-block',
              }}
            >
              전화 걸기
            </a>
            <a
              href={KAKAO_URL}
              target="_blank"
              rel="noreferrer"
              style={{
                background: '#FEE500',
                color: '#3C1E1E',
                fontWeight: 700,
                fontSize: 15,
                padding: '13px 26px',
                borderRadius: 4,
                display: 'inline-block',
              }}
            >
              카카오톡 상담
            </a>
          </div>
        </div>
        <form
          ref={formRef}
          onSubmit={submitForm}
          style={{
            background: '#FFFFFF',
            border: '1px solid rgba(112,115,124,.22)',
            borderRadius: 12,
            padding: '36px 32px',
            display: 'flex',
            flexDirection: 'column',
            gap: 18,
          }}
        >
          {sent && (
            <div
              style={{
                background: 'rgba(195,161,95,.12)',
                border: '1px solid #C3A15F',
                borderRadius: 8,
                padding: '14px 18px',
                fontSize: 15,
                color: '#14263F',
              }}
            >
              문의가 접수되었습니다. 빠르게 연락드리겠습니다.
            </div>
          )}
          <div style={fieldWrapStyle}>
            <label style={labelStyle}>성함</label>
            <input required name="name" placeholder="홍길동" style={inputStyle} />
          </div>
          <div style={fieldWrapStyle}>
            <label style={labelStyle}>연락처</label>
            <input required name="phone" placeholder="010-0000-0000" style={inputStyle} />
          </div>
          <div style={fieldWrapStyle}>
            <label style={labelStyle}>문의 내용</label>
            <textarea
              required
              name="msg"
              rows={4}
              placeholder="기장대리 견적 문의드립니다."
              style={{ ...inputStyle, resize: 'vertical' }}
            />
          </div>
          <button
            type="submit"
            className="lc-btn-gold"
            style={{
              color: '#14263F',
              fontWeight: 700,
              fontSize: 16,
              padding: 15,
              borderRadius: 6,
              border: 'none',
              cursor: 'pointer',
              fontFamily: 'inherit',
            }}
          >
            상담 신청하기
          </button>
        </form>
      </div>
    </div>
  )
}
